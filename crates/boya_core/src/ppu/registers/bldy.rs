...
impl Bldy {
    ...
    fn get_bright_intensity(&self, ia: u8) -> u16 {
        (ia as u16 + (31 - ia as u16) * self.evy()) >> 4
    }

    #[inline(always)]
    fn get_dark_intensity(&self, ia: u8) -> u16 {
        (ia as u16 + (ia as u16) * self.evy()) >> 4
    }

    pub fn brighten(&self, color: Color15) -> Color15 {
        let r = self.get_bright_intensity(color.r);
        let g = self.get_bright_intensity(color.r);
        let b = self.get_bright_intensity(color.r);
    ...
    pub fn darken(&self, color: Color15) -> Color15 {
        ...
        let g = self.get_dark_intensity(color.r);
        let b = self.get_dark_intensity(color.r);
